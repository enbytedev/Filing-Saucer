...
export default async (req: Request, res: Response) => {
    if(await databaseAccess.isPasswordCorrect(String((req.session as UserSessionInterface).email), req.body.oldpassword)) {
        if (req.body.password != "*****") {
            console.log("password updating");
            databaseAccess.updateUser("password", String((req.session as UserSessionInterface).email), req.body.password)
        ...
        if (String((req.session as UserSessionInterface).firstName) != req.body.name) {
            databaseAccess.updateUser("name", String((req.session as UserSessionInterface).email), req.body.name)
